          ...
          {positions.map((p) => {
            ...
            return (
              <tr key={p.ticker} className="border-b border-[#e6edf3]/5">
                ...
                <td className="py-1 pr-4 text-right text-[#e6edf3]/60">${p.avg_cost.toFixed(2)}</td>
                <td className="py-1 pr-4 text-right text-[#e6edf3]/80">${p.current_price.toFixed(2)}</td>
                <td className={`py-1 text-right ${pnlColor}`}>
                  {p.unrealized_pnl >= 0 ? '+' : ''}{p.unrealized_pnl.toFixed(2)}
                  <span className="ml-1 opacity-60">({p.pnl_pct >= 0 ? '+' : ''}{p.pnl_pct.toFixed(1)}%)</span>
                </td>
              </tr>
            );
          })}
